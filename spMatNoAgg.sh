#!/bin/bash
#SBATCH --job-name=spMatNoAgg
#SBATCH --output=spMatNoAgg_%A_%a.out
#SBATCH --error=spMatNoAgg_%A_%a.err
...
# Validate arguments
if [[ -z "$X" || -z "$Y" || -z "$Z" ]]; then
    echo "Usage: sbatch --nodes=$X spMatNoAgg.sh X Y Z"
...
OUTPUT_FILE="spMatNoAgg_${X}nl_${Y}s_${Z}t.txt"
...
/lus/scratch/alvarado/chapel/sparseMatrixCreationBenchmark -nl $X --SCALE=$Y --trials=$Z --runAgg=false > $OUTPUT_FILE 2>&1
